"""Manual real-pipeline scoring of fusion_v1 capture f5482887 (2026-09-03
04:47 UTC) -- first real capture on the camera-0 macro fix
(macroCameraName == "0") plus the physicalCameraIds diagnostic. Same
manual-harness pattern as every other fusion_v1 capture this session
(production trigger is off for this experimental mode).
"""
...
CID = 'f5482887-692e-4585-b404-7f43af4eb6ab'
CACHE = os.path.join(HERE, 'results', 'cache', 'f5482887')
os.makedirs(CACHE, exist_ok=True)
IMGDIR = '/tmp/f5482887_imgs'
...
print("=== cameraLensInfo / physicalCameraIds ===")
for cid, info in sorted(d.get('cameraLensInfo', {}).items()):
    phys = d.get('physicalCameraIds', {}).get(cid)
    print("  cam %s: afAvailableModes=%s  focalLengthMm=%.2f  lensFacing=%s  physicalSubIds=%s" % (
        cid, info.get('afAvailableModes'), info.get('focalLengthMm', -1),
        info.get('lensFacing'), phys))
...
print("\n=== MACRO (camera 0, main guide reused) ===")
...
if m_amb is not None:
    ...
    for name, kw in VARIANTS:
        img, p = afis_print.generate([m_amb], [0.0], ['ambient'], guide_region=guide,
    ...
    h, w = m_amb.shape
    cx_px, cy_px = int(guide['cx']*w), int(guide['cy']*h)
    rx_px, ry_px = int(guide['rx']*w*1.3), int(guide['ry']*h*1.3)
...
print("\n=== ULTRA-WIDE SWEEP ZONES (camera 2, tag cam3_sweep_*) ===")
uw = {f['tag']: fetch(f['path']) for f in d.get('cam3Shots', [])}
for zone, gkey in (('left', 'cam3_sweep_left'), ('center', 'cam3_sweep_center'), ('right', 'cam3_sweep_right')):
    za = uw.get('%s_amb' % gkey)
    g = fgr.get(gkey)
    if za is None or g is None: continue
    zf = uw.get('%s_fl' % gkey)
    fl_list = [zf] if zf is not None else [za]
    ...
    s = nfiq2(img)
    print("  %-8s nfiq2=%s mask=%s" % (zone, s, p.get('afisMask')))
    out = os.path.join(IMGDIR, 'uw_%s.png' % zone)
    if img is not None:
        cv2.imwrite(out, img)
