"""Layer 3, the question underneath all the detector work: does content-aware
...
for cid,d,g,fr in caps:
    ...
    afis_print._flash_diff_mask, afis_print._unet_mask = _FD, _UN
    pv,bv = res['prod'][0], res['bare'][0]
    dl = ('%+.0f'%(pv-bv)) if (pv is not None and bv is not None) else '?'
    print("  %s  prod=%-6s %-18s  bare=%-6s   refine delta=%s"
          % (cid[:8], pv, res['prod'][1], bv, dl), flush=True)
    out.append(dict(id=cid[:8], prod=pv, prod_mask=res['prod'][1], bare=bv))
    json.dump(out, open(os.path.join(HERE,'results','mask_refinement_value.json'),'w'), indent=1)

pair=[(o['prod'],o['bare'],o['prod_mask']) for o in out if o['prod'] is not None and o['bare'] is not None]
if pair:
